...
### Gemini API 키 관리
- **Workers Secrets** (`wrangler secret put GEMINI_API_KEY`) — 프론트 절대 노출 금지
- 로컬: `backend/.dev.vars` (gitignored), 템플릿은 `backend/.dev.vars.example`
- Worker 가 `/api/coach/*`, `/api/train/realtime-adjust` 등 자체 엔드포인트로 프록시
- Gemini 클라이언트 래퍼 위치: `backend/src/lib/gemini.ts`
...
| LLM JSON 검증 | **Zod** | Gemini structured output → Zod 강검증 → 재시도 |

### 함의
- `backend/package.json` dependencies: `hono`, `@hono/zod-validator`, `zod`, `drizzle-orm`, `@google/generative-ai` (또는 fetch 직접)
...
- **Gemini 호출 비용 모니터링**: Worker 내 호출 카운터 + 일일 한도 (Phase 2 진입 시 결정)
