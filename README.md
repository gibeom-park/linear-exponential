...
파워리프팅 프로그램 도구. Cloudflare Workers + D1 + React PWA.

설계 문서는 [`plan/claude/`](./plan/claude/) (도메인 / 인프라 / 로드맵), 도메인 자료는 [`docs/`](./docs/). 1차안의 LLM 파이프라인 설계는 [`plan/claude/archive/prompts_model.md`](./plan/claude/archive/prompts_model.md) 에 보존 (AI 도입은 Post-MVP).
...
3. 환경 변수 파일 복사:
   ...
   ```
   Phase 2 (수동 코치 모드) 에는 Worker 측 외부 API 키가 필요 없음 — `backend/.dev.vars` 는 비워두거나 만들지 않아도 됨.
...
| `backend/.dev.vars` (gitignored) | (Phase 2 기준 비어 있음) | `wrangler dev` 가 Worker 에 주입 |
| Cloudflare Workers Secrets | (Phase 2 기준 없음) | 배포된 Worker |

Phase 2 (수동 코치 모드) 는 외부 LLM 호출이 없어 Worker 시크릿이 필요 없음. 1차안에서 등록한 `GEMINI_API_KEY` 가 남아 있다면 정리:
```sh
pnpm --filter @linex/backend exec wrangler secret delete GEMINI_API_KEY
```
AI 재도입 (Post-MVP) 시 시크릿 관리 방식은 다시 결정 — `plan/claude/archive/prompts_model.md` 참조.
...
├── backend/    # Cloudflare Worker + Hono + Drizzle + D1
...
└── plan/       # 설계 문서
    ...
    └── claude/                # Claude 작성 (domain / infra / roadmap)
        └── archive/           # 보존용 (1차 LLM 파이프라인 설계 등)
```
